...
const Exams = () => {
  ...
  const examPost = posts.filter((post)=>{
    return post.post.category == "Next Exam"
  })
      ...
      <div className="row px-5">
        <div className="w-100 py-2">
          <div className="latestPostsHeading border-bottom border-primary d-flex mx-5">
            <p style={{background:"black"}} className=" text-white col-md-12 d-flex align-items-center justify-content-center py-2 h5">
              All Updates on Next Exams
            </p>
          </div>
          <div className="row my-5">
            {examPost.length == 0 && <p>No Update on Next Exams available</p>}
            {postsLoading
              ? "Loading posts"
